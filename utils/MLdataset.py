...
def loadMfDIMvMlDataFromMat(mat_path, fold_mat_path,fold_idx=0):
    ...
    if 'X' in data:
        ...
        mv_data = data['views'][0]
    elif 'data' in data:
        mv_data = data['data'][0]
    else:
        raise ValueError('Cannot find view data variable.')
    ...
    if 'Y' in data:
        ...
        raise ValueError('Cannot find label variable.')
...
class IncDataset(Dataset):
    def __init__(self,inc_mv_data, inc_labels, labels, inc_V_ind, inc_L_ind, total_sample_num, training_ratio=0.7, val_ratio=0.1, fold_idx=0, mode='train',semisup=False):
...
def getIncDataloader(inc_mv_data, inc_labels, labels, inc_V_ind, inc_L_ind, total_sample_num, training_ratio=0.7, val_ratio=0.1, fold_idx=0, mode='train',batch_size=1,num_workers=1,shuffle=False):
    dataset = IncDataset(inc_mv_data, inc_labels, labels, inc_V_ind, inc_L_ind, total_sample_num, training_ratio=training_ratio, val_ratio=val_ratio, mode=mode, fold_idx=fold_idx)
    dataloder = DataLoader(dataset=dataset,batch_size=batch_size,shuffle=shuffle,num_workers=num_workers)
    return dataloder,dataset
